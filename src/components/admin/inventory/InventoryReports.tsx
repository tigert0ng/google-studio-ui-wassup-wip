              ...
              <tbody className="divide-y divide-stone-100 text-[11.5px] text-slate-600">
                {turnoverItems.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3 text-right">{formatVnd(item.costOfSales)}</td>
                    <td className="p-3 text-right">{formatVnd(item.avgInv)}</td>
                    <td className="p-3 text-center font-extrabold text-slate-800">{item.turnover}</td>
                    <td className="p-3 text-center font-extrabold text-slate-800">{item.days} ngày</td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 text-white p-5 rounded-2xl shadow-sm border border-slate-800 flex flex-col justify-between">
            <div className="space-y-1">
              ...
              <h4 className="text-2xl font-black text-white">{formatVnd(totalInventoryValue)}</h4>
            </div>
            ...
            <div className="space-y-3 pt-1 text-xs">
              <div className="space-y-1">
                <div className="flex justify-between items-center text-[11px]">
                  ...
                  <span className="text-slate-500">{formatVnd(totalCommercialValue)} ({((totalCommercialValue / (totalInventoryValue || 1)) * 100).toFixed(1)}%)</span>
                </div>
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center text-[11px]">
                  ...
                  <span className="text-slate-500">{formatVnd(totalConsumableValue)} ({((totalConsumableValue / (totalInventoryValue || 1)) * 100).toFixed(1)}%)</span>
                </div>
                <div className="w-full h-1.5 bg-stone-100 rounded-full overflow-hidden">
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center text-[11px]">
                  ...
                  <span className="text-slate-500">{formatVnd(totalToolValue)} ({((totalToolValue / (totalInventoryValue || 1)) * 100).toFixed(1)}%)</span>
                </div>
                <div className="w-full h-1.5 bg-stone-100 rounded-full overflow-hidden">
